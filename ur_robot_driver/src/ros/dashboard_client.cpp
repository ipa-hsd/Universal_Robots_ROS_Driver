...
#ifndef ADVERTISE_TRIGGER_CALL
#define ADVERTISE_TRIGGER_CALL(topic, command, expected)                                                               \
  ...
  brake_release_service_ = ADVERTISE_TRIGGER_CALL("brake_release", "brake release\n", "Brake releasing");

  // If this service is called the operational mode can again be changed from PolyScope, and the user password is
  // enabled.
  clear_operational_mode_service_ = ADVERTISE_TRIGGER_CALL("clear_operational_mode", "clear operational mode\n",
                                                           "No longer controlling the operational mode\\. Current "
                                                           "operational mode: "
                                                           "'(MANUAL|AUTOMATIC)'\\.");

  // Close a (non-safety) popup on the teach pendant.
  close_popup_service_ = ADVERTISE_TRIGGER_CALL("close_popup", "close popup\n", "closing popup");

  // Close a safety popup on the teach pendant.
  close_safety_popup_service_ =
      ADVERTISE_TRIGGER_CALL("close_safety_popup", "close safety popup\n", "closing safety popup");

  // Pause a running program.
  pause_service_ = ADVERTISE_TRIGGER_CALL("pause", "pause\n", "Pausing program");

  // Start execution of a previously loaded program
  play_service_ = ADVERTISE_TRIGGER_CALL("play", "play\n", "Starting program");

  // Power off the robot motors
  power_off_service_ = ADVERTISE_TRIGGER_CALL("power_off", "power off\n", "Powering off");

  // Power on the robot motors. To fully start the robot, call 'brake_release' afterwards.
  power_on_service_ = ADVERTISE_TRIGGER_CALL("power_on", "power on\n", "Powering on");

  // Disconnect from the dashboard service. Currently, there's no way of reconnecting.
  quit_service_ = ADVERTISE_TRIGGER_CALL("quit", "quit\n", "Disconnected");
  ...
  restart_safety_service_ = ADVERTISE_TRIGGER_CALL("restart_safety", "restart safety\n", "Restarting safety");

  // Shutdown the robot controller
  shutdown_service_ = ADVERTISE_TRIGGER_CALL("shutdown", "shutdown\n", "Shutting down");

  // Stop program execution on the robot
  stop_service_ = ADVERTISE_TRIGGER_CALL("stop", "stop\n", "Stopped");
  ...
  unlock_protective_stop_service_ =
      ADVERTISE_TRIGGER_CALL("unlock_protective_stop", "unlock protective stop\n", "Protective stop releasing");
}
